  ...
  const handleSubmitCreateClient = async (e) => {
    if (theId) {
      ...
      try {
        ...
        await api.put(`/regExternal/${iId}`, { name, firstNumber });

        setClientsMain(
          clients.map((client) =>
            client._id === iId ? { name, firstNumber, _id: iId } : client
          )
        );
...
*/
        ...
        const { data } = await api.post("/regExternal", { name, firstNumber });
