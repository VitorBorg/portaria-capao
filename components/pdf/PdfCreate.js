...
function pdfFile(date, info, external, user, backupTypePdf) {
  ...
  const dadosExternal = external.map((client) => {
    return [
      ...
      {
        text: client.createdAt.substring(0, 10).split("-").reverse().join("/"),
        fontSize: 9,
        margin: [0, 2, 0, 2],
      },
    ];
  ...
  const dados = info.map((client) => {
    return [
      {
        text: user.filter((userData) => userData._id == client.name)[0].name,
        fontSize: 9,
        margin: [0, 2, 0, 2],
      },
...
function PdfCreate({
  ...
  const creating = () => {
    if (
        ...
        !processingDone && (
          <div className="container">
            <div className="flex flex-wrap mb-5 mt-5 content-center">
              <a
                className="px-4 py-2 text-white font-semibold rounded bg-blue-500 cursor-pointer"
                href="/Export"
              >
                Concluir processo
              </a>
            ...
            {backupTypePdf == 0 && (
              <button
                className="flex flex-wrap px-4 py-2 text-white font-semibold bg-red-400 rounded
                  content-center"
                onClick={() => (
      ...
      <div>{detl("Buscando registros no banco e montando o pdf...")}</div>;
    }
  };
