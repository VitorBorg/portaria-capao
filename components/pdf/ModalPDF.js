...
function ModalPDF({ onClose, backupType }) {
  const [databaseClientsInternal, setDatabaseClientsInternal] =
    useState(undefined);
  const [databaseClientsExternal, setDatabaseClientsExternal] =
    useState(undefined);
  const [databaseClientsUsersData, setDatabaseClientsUsersData] =
    useState(undefined);
      ...
      if (backupType == 0) {
        api.get("/regExternal").then(({ data }) => {
          setDatabaseClientsExternal(
            data.data.filter(
              (client) => strcmp(client.createdAt.substring(5, 7), date) == 0
            )
          );
        });
        ...
        api.get("/regExternal").then(({ data }) => {
          setDatabaseClientsExternal(
            data.data.filter(
              (client) => strcmp(client.createdAt.substring(0, 10), date) == 0
            )
          );
        });
          ...
          api.get("/regInternal").then(({ data }) => {
            setDatabaseClientsInternal(
              data.data.filter(
                (client) =>
                  strcmp(
                    databaseClientsUsersData.filter((userData) =>
                      userData.name.toLowerCase().includes(date.toLowerCase())
                    )[0]._id,
                    client.name
                  ) == 0
              )
            ...
            <div>
              ...
              <div className="flex flex-col px-6 py-5 bg-gray-50">
                <div className="flex flex-col w-full items-center mb-5">
                  {finished && (
                    <h1 className="mb-3">BACKUP FEITO COM SUCESSO</h1>
                  )}
                      ...
                      <div>
                        ...
                        <div className="flex flex-col px-6 py-5 bg-gray-50">
                          ...
                          <p className="font-semibold text-gray-800 mb-5">
                            O usuário que você busca é um visitante?
              ...
              <div className="flex flex-row items-center justify-between p-5 bg-white border-t border-gray-200 rounded-bl-lg rounded-br-lg">
                <div className="pr-2">
                  {!loading && !finished && (
                    <button
                      className="px-4 py-2 text-white font-semibold bg-red-400 rounded
            "
                      onClick={() => (setFinished(false), onClose())}
                    >
                      Cancelar
